...
fun getMoneySpent(keyboards: Array<Int>, drives: Array<Int>, b: Int): Int {
    val affordableKeyboards = keyboards.filter { it < b }
    val affordableDrives = drives.filter { it < b }
    var bestBuy = -1
    affordableKeyboards.forEach { k ->
        affordableDrives.forEach { d ->
            val currentBuy = d + k
            if(currentBuy in (bestBuy + 1)..b) {
                bestBuy = currentBuy
            }
        }
    }
